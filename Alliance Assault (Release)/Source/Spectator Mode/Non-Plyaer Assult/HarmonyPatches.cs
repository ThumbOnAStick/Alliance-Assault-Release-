...
namespace Non_Plyaer_Assault
{
    ...
    public static class SettlementShouldRemoveMapPatch
    {
        ...
        {
            var aa = __instance.GetComponent<AllianceAssaultComp>();
    ...
    public static class SiteShouldRemoveMapPatch
    {
        ...
        public static void Postfix(ref bool __result, Site __instance)
        {
            ...
            var basicObserver =__instance.GetComponent<BasicObserverComp>();
            if (basicObserver != null&& basicObserver.IsObservation)
            {
                __result = false;
                return;
            }
    ...
    public static class SiteGetGizmosPatch
    {
        [HarmonyPostfix]
        public static void Postfix(ref IEnumerable<Gizmo> __result, Site __instance)
        {
            var basicObserver = __instance.GetComponent<BasicObserverComp>();
            var factionalWarObserver = __instance.GetComponent<FactionWarObserverComp>();

            if (factionalWarObserver != null)
            {
                if (!__instance.HasMap)
                {
                    __result = __result.AddItem(AllianceAssultUtility.ObserveFactionalWar(__instance));
                }
                else
                {
                    __result = __result.AddItem(AllianceAssultUtility.StopObservingFactionalWar(__instance));
                }
                return;
            }
      ...
      .Where(x => x != Faction.OfPlayer && x.RelationKindWith(Faction.OfPlayer) == FactionRelationKind.Ally).ToList();
                if (factions.Count < 1)
                {
                    return;
                }

                __result = __result.AddItem(AllianceAssultUtility.AllianceAssaultCommand(__instance));
                basicObserver.IsObservation = true;
            }
    ...
    public static class SettlementDefeatPatch
    {
        ...
        public static void DefeatSettlement( Settlement factionBase)
        {
            ...
            //alliance assault
            var aa = factionBase.GetComponent<AllianceAssaultComp>();
            ...
            if (aa!=null&&aa.isAllianceAssault)
            {
                ...
                if (factionBase.def.canBePlayerHome && factionBase.def.canHaveFaction && !aa.ally.Hidden)
                {
