...
namespace Non_Plyaer_Assault
{
    ...
    public static class AllianceAssultUtility
    {
        ...
        public static Command CancelAttackCommand(MapParent map)
        {
            Command_Action command_Action = new Command_Action();
            ...
            command_Action.action = delegate ()
            {
                if (map.Map != null && !map.Map.mapPawns.AnyColonistSpawned)
                {
                    var aa = map.GetComponent<AllianceAssultComp>();
                    aa.isAllianceAssault = false;
                    Current.Game.DeinitAndRemoveMap_NewTemp(map.Map, false);
                }
            };
            return command_Action;
        }

        public static Command StopObservingDestroyedSettlement(MapParent mapP)
        {
            Command_Action command_Action = new Command_Action();
        ...
        {
            //Generate Map
            var aa = mapP.GetComponent<AllianceAssultComp>();
            if (aa == null)
            {
                return;
            }
